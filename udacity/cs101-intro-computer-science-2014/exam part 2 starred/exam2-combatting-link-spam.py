...
def is_reciprocal(page, node, graph, k):
    """ Return True if node can be reached from
       page within k hops, False otherwise. """
    if k is 0:
        return True if node == page and page in graph[page] else False
    found = [] + graph[page] # Prevent altering of graph.
    i = 0
    while k - 1:
        stop = len(found)
        while i < stop:
            union(found, graph[found[i]])
            i += 1
        k -= 1
    return True if node in found else False
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not is_reciprocal(page, node, graph, k):
