...
elements = [128, 64, 32, 16, 8, 4, 2, 1]
patterns = ['xxx','xx.','x.x','x..','.xx','.x.','..x','...']

#analyze an integer into sum of power of 2
#parse_number(7) = [4,2,1]
def parse_number (n):
    res = list ()
    tmp = n
...
def cellular_automaton(inp, pattern, generation):

    pattern_list = parse_number (pattern)
    
    #temporary string to process
    s = inp
    
    #calculate one time, because it is constant
    length = len (inp)
    for i in range (generation):
        ...
        for j in range (length):
            ...
            for index in range (len(patterns)):
                if tmp == patterns [index]:
                    if index in pattern_list:
                        tmpStr = tmpStr + "x"
                    else:
                        tmpStr = tmpStr + "."
        s = tmpStr
